...
Cube cubes[]{
        ...
        {{1.0f, 1.0f, 1.0f}, glm::vec3(1.0f, 0.0f, -2.0f)}
};
...
void rotate(Shader &cubeShader, Shader &lightShader) {
    ...
    cubes[1].color[0] = sin(time * rd());
    cubes[1].color[1] = sin(time * rd());
    cubes[1].color[1] = sin(time * rd());

    float_t viewPos[]{0.0f, 0.0f, -4.0f};
...
//            glUniform3f(glGetUniformLocation(cubeID, "lightPos"), cubes[1].position.x, cubes[1].position.y, cubes[1].position.z);
            ...
            int r = 2;
            cube.position.x = sin(time) * r;
            cube.position.z = cos(time) * r;
            model = glm::mat4{1.0f};
            model = glm::translate(model, cube.position);
            model = glm::rotate(model, glm::radians(-50.0f), glm::vec3(0.0f, 1.0f, 0.0f));
...
void loadTexure() {
    ...
    u_char *data = stbi_load("../resources/container2.png", &width, &height, &nrChannels, 0); // 加载图片

    // 生成纹理
    u_int texture1, texture2;
    glGenTextures(1, &texture1); // 生成一个纹理
    glBindTexture(GL_TEXTURE_2D, texture1); // 绑定到2D纹理缓冲
    ...
    data = stbi_load("../resources/container2_specular.png", &width, &height, &nrChannels, 0); // 加载图片
    ...
    // 将texture0和GL_TEXTURE0关联，间接关联到纹理0号单元
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, texture1);

    // 将texture1和GL_TEXTURE1关联，间接关联到纹理0号单元
...
int main() {
    ...
    std::string vCubeShaderPath = "../glsl/light_tietu_v.glsl";
    std::string fCubeShaderPath = "../glsl/light_tietu_f.glsl";
    std::string vLightShaderPath = "../glsl/triangle_v_light.glsl";
    ...
    loadTexure();
    // 设置纹理单元
    glUniform1i(glGetUniformLocation(cubeShader.getID(), "material.diffuse"), 0);
    glUniform1i(glGetUniformLocation(cubeShader.getID(), "material.specular"), 1);
